...
@Dao
interface DrillDao {
  @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(drill: DbDrill)

  @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insertAll(drills: List<DbDrill>)

  @Query("DELETE from drill where day = :day") suspend fun deleteDay(day: Long)
